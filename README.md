...
The known lifts, in rough order: per-show config (`bin/<show>/config.json` per
SKILL.md), de-hardcoding the two-host assumptions in `director/`, an API path
for the audio host, and generalizing the thumbnail branding. Today it's a
working pipeline for one show, shipping weekly.
